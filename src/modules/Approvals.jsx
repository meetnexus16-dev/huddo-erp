...
export default function Approvals({ showToast }) {
  ...
  const pendingColumns = [
    ...
    { header: 'Details', accessor: 'details', render: (val) => <span className="block max-w-[260px] truncate text-xs text-slate-500" title={val}>{val}</span> },
      ...
      render: (val, row) => (
        ...
        <div className="flex gap-2 items-center">
          ...
          <button
            ...
            className="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white disabled:opacity-60"

            title="Approve"

            aria-label={`Approve ${row.requester}`}
          ...
          <button
            ...
            className="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-rose-600 hover:bg-rose-700 text-white disabled:opacity-60"

            title="Reject"

            aria-label={`Reject ${row.requester}`}
